...
def generate_points(rad,ntot):
    ncount = 0
    pi = math.pi
    a = 4*pi*abs(rad)**2/ntot
    d = math.sqrt(a)
    mtheta = round(pi*abs(rad)/d)
    dtheta = pi*abs(rad)/mtheta
    dphi = a/dtheta
    coords = []
    angles = []
    ...
    for m in range(mtheta):
        ...
        mphi = round(2*pi*abs(rad)*math.sin(theta)/dphi)
        for n in range(mphi):
            phi = 2*pi*n/mphi
...
def flatBilayer(system, numA, numB=None, verbose=False, z0=2.0):
    ...
    dAngle = np.array([[np.pi,0.]]*len(sites[1]))
...
###same as above but for 3 types of lipids
def leafletContent3types(system,lipids,typeAt,typeBt,typeAb,typeBb,typeC,typeD):
    topA   = 0
    topB   = 0
    topC   = 0
    topD   = 0
    ...
    for l in lipids:
        if(not stray(system, l)):
            ...
            x = np.dot(tth, unitZ)
            if x < 0: # it is in the bottom
                if l.type == typeAb or l.type == typeAt:
                    botA += 1
                elif l.type == typeBb or l.type == typeBt:
                    botB += 1
                elif l.type == typeC:
                    botC += 1
                elif l.type == typeD:
    ...
    return (topA, botA, topB, botB, topC, botC,topD,botD, nStray)
    
def leafletContent_generic(system,lipids,types):
    top = np.zeros(len(types))
    bot = np.zeros(len(types))
    nStray = 0
    unitZ = np.array([0,0,1])
    ...
    for l in lipids:
        if(not stray(system, l)):
            ...
            nStray += 1
    list = [top[i] for i in range(len(top))] + [bot[i] for i in range(len(bot))] + [nStray]
